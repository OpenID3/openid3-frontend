...
const getNonce = async (accountAddr: string, chain: Chain) => {
...
export const buildZkAdminData = (admin: GoogleZkAdmin, accountHash: string) => {
  let adminData = admin.interface.encodeFunctionData("linkAccount", [
    accountHash,
  ]);
...
export async function buildAdminCallResetOperatorUserOp(
  ...
  const callData = iface.encodeFunctionData("setOperator", [newOperator]);
